...
# Use the app's own DATABASE_URL rather than whatever's hardcoded in
# alembic.ini, so `alembic upgrade head` always targets the same DB the
# gateway itself is configured to use.
...
async def run_async_migrations() -> None:
    """Open an async engine from DATABASE_URL and run the migrations on one connection."""
