...
- **Dashboards** — `docker compose up` brings up Prometheus (`:9090`) scraping the gateway and Grafana (`:3000`, default login `admin`/`admin`), auto-provisioned with a "LLM Gateway Overview" dashboard — no manual setup, it's already there on first load. 12 panels: request rate by status, p50/p95/p99 latency, provider attempts by outcome, fallback rate, **circuit breaker state per provider** (colour-mapped closed/half-open/open), **per-provider p95** (the global latency histogram covers retries and every fallback hop, so it can't compare providers), **spend rate in USD/hour**, **cumulative spend by model**, **token throughput by direction**, **lifetime spend vs headroom** — answering "how much is left" rather than "how much has gone", which is the version worth alerting on — and **refusals by reason**, which is the only place a request the gateway turned away shows up at all, since a refusal raises before anything reaches the request counter. The twelfth is **unhandled exceptions**, for the same reason one step further out: a 500 raised before or around a handler reaches no counter either, so the request rate simply flattens — which looks exactly like nobody calling. A test asserts every metric the dashboard queries actually exists — a panel pointing at a renamed metric renders empty rather than failing, which looks identical to "no traffic yet".
- **Alerting** — 11 rules in `deploy/prometheus/alerts.yml`, loaded by the bundled Prometheus, each carrying a `runbook_url` that points at the matching section of [`docs/runbook.md`](docs/runbook.md). They cover four different questions. **Is a provider down** — `ProviderCircuitOpen`, plus `ProviderCircuitDisagreement` for when replicas disagree, which is the signature of a replica-local fault rather than a provider outage and which a shared breaker would have hidden by design. **Is money running out** — `ProviderBudgetLow`, `ProviderBudgetExhausted`, `GatewayRefusingUnpricedRequests` for a model whose cost can't be bounded, which is an operator misconfiguration rather than a spend problem, and `BudgetReservationLeaked` for a reservation that was claimed and couldn't be handed back. That last one fires on any leak at all, because it fails in the safe direction — the ceiling gets smaller, never larger — and that is exactly what would stop anyone noticing: spend looks normal while headroom quietly erodes, and Redis holds the only copy of the number. **Is the gateway itself the problem** — `GatewayTargetDown`, because every other rule reads metrics the gateway publishes and so none of them fire when the gateway is what's broken, and `GatewayRequestsFailingAcrossWholeChain`, the outcome fallback exists to prevent. And **is it failing in a way nothing else sees** — `ProviderFallbackRateHigh` and `RequestLatencyDegraded`, the two failure modes the other rules structurally miss, plus `UnhandledExceptions` for 500s, which reach no request counter at all because the counter is incremented inside the handlers. The first two catch the case where a provider that's degraded rather than failing never trips the breaker, so requests keep succeeding while every one of them pays for a failed attempt first. Tests assert each rule references a metric that exists, filters on refusal reasons the code actually emits, carries a severity Alertmanager can route on, and links to a runbook section that's really there — an alert querying a renamed metric never fires, and never firing is indistinguishable from healthy.
