...
- **Alerting** — 9 rules in `deploy/prometheus/alerts.yml`, loaded by the bundled Prometheus, each carrying a `runbook_url` that points at the matching section of [`docs/runbook.md`](docs/runbook.md). They cover four different questions. **Is a provider down** — `ProviderCircuitOpen`, plus `ProviderCircuitDisagreement` for when replicas disagree, which is the signature of a replica-local fault rather than a provider outage and which a shared breaker would have hidden by design. **Is money running out** — `ProviderBudgetLow`, `ProviderBudgetExhausted`, and `GatewayRefusingUnpricedRequests` for a model whose cost can't be bounded, which is an operator misconfiguration rather than a spend problem. **Is the gateway itself the problem** — `GatewayTargetDown`, because every other rule reads metrics the gateway publishes and so none of them fire when the gateway is what's broken, and `GatewayRequestsFailingAcrossWholeChain`, the outcome fallback exists to prevent. And **is it working but badly** — `ProviderFallbackRateHigh` and `RequestLatencyDegraded`, the two the others structurally miss: a provider that's degraded rather than failing never trips the breaker, so requests keep succeeding while every one of them pays for a failed attempt first. Tests assert each rule references a metric that exists, filters on refusal reasons the code actually emits, carries a severity Alertmanager can route on, and links to a runbook section that's really there — an alert querying a renamed metric never fires, and never firing is indistinguishable from healthy.
